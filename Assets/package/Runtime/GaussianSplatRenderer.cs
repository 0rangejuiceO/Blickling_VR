...
namespace GaussianSplatting.Runtime
{
    class GaussianSplatRenderSystem
    {
        ...
        public Material SortAndRenderSplats(Camera cam, CommandBuffer cmb)
        {
            ...
            foreach (var kvp in m_ActiveSplats)
            {
                ...
                // sort
                var camTr = cam.transform;
                float angleChange = Quaternion.Angle(gs.m_LastSortCameraRot, camTr.rotation);

                if (gs.m_FrameCounter >= gs.m_SortNthFrame || angleChange > gs.m_SortOnRotation)
                {
                    gs.SortPoints(cmb, cam, matrix);
                    gs.m_LastSortCameraRot = camTr.rotation; // Update rotation after sorting
                    gs.m_FrameCounter = 0; // Reset frame counter
                }
                ++gs.m_FrameCounter;
    ...
    public class GaussianSplatRenderer : MonoBehaviour
    {
        ...
        [Range(1,2000)] [Tooltip("Sort splats only every N frames")]
        public int m_SortNthFrame = 120;

        [Tooltip("Sort splats if camera rotates by more than this many degrees since last sort.")]
        public float m_SortOnRotation = 90.0f;
        ...
        internal Quaternion m_LastSortCameraRot;
        ...
        {
            m_FrameCounter = 0;
            m_LastSortCameraRot = Quaternion.identity;
